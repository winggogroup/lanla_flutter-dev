...
import 'package:lanla_flutter/models/Position.dart';
...
import 'package:lanla_flutter/pages/user/NewMobileverification/passwordlogin.dart';
...
class MobileverificationPage extends StatefulWidget {
  @override
  createState() => MobileverificationState();
}
...
class MobileverificationState extends State<MobileverificationPage> {
  ...
  ///获取定位
  hqpositioning() async {
  var result = await  provider.positions();

    Map<String, dynamic> responseData =
        jsonDecode(result?.bodyString.toString() ?? '');
  ...
  testverify() async {
    if(antishake){
      ...
      var result = await provider.checkStatus(phonenumber,areacode);
      if(result.statusCode==200){
        print('shuju${result.body['status']}');
        Navigator.pop(context);
        if(result.body['status']=='phone_register_forbidden'){
          ///禁止手机注册
          Get.offAll(HomePage());
          Prohibitregistration(context);
        }
        if(result.body['status']=='to_password_login'){
          ///去登录
          Get.to(passwordloginPage(),arguments: {'phone':phonenumber,'areacode':areacode});
        }
        if(result.body['status']=='need_security_authentication'){
          ///设置密码
           Get.to(SecurityverificationPage(),arguments: {'phone':phonenumber,'areacode':areacode,'type':1});
        }
        if(result.body['status']=='security_authentication_forbidden'){
          ///操作太多了
          Get.offAll(HomePage());
          customer(context);
        }
      ...
      body: Container(
        ...
        child: ListView(
          ...
          children: [
            ...
            Text('欢迎你的到来'.tr,style:const TextStyle(fontSize: 25,fontWeight: FontWeight.w600),),
                        ...
                        Row(
                          ...
                          showCountryPicker(
                            ...
                            onSelect: (Country country) {
                              areacode=country.phoneCode;
                              favorite=country.countryCode;
                              setState(() {});
                            },
            ...
            Container(
              ...
              child: ElevatedButton(
                ...
                onPressed: () async {
                  ...
                  //Get.to(SecurityverificationPage());
  ...
  Future<void> Prohibitregistration(context) async {
